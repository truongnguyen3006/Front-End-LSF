  ...
  return (
    <div className="app-shell animate-fade-in py-8 md:py-10">
      <div className="mb-8 flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
        <div>
          <div className="text-xs font-semibold uppercase tracking-[0.18em] text-[var(--color-muted)]">Đơn hàng của tôi</div>
          ...
          <p className="mt-3 max-w-2xl text-sm leading-6 text-[var(--color-secondary)] md:text-base">
            Các hệ thống thương mại điện tử hiện đại thường hiển thị thanh tiến độ đơn hàng. Màn này cũng áp dụng cách đó để người dùng dễ biết đơn đang ở giai đoạn nào.
          </p>
        </div>
        <Link href="/products" className="app-secondary-btn">
          Tiếp tục mua sắm
      ...
      {isLoading ? (
        ...
        <div className="app-surface px-6 py-10">
          <Result status="warning" title="Chưa thể tải danh sách đơn hàng" subTitle="Endpoint hiện tại có thể chưa hỗ trợ đầy đủ cho tài khoản khách hàng." />
        </div>
                ...
                <div className="flex flex-col gap-5">
                  <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                    <div>
                      ...
                      <div className="mt-2 text-xl font-semibold tracking-tight text-[var(--color-primary)]">{order.orderNumber}</div>
                      <div className="mt-2 text-sm text-[var(--color-secondary)]">
                        {order.orderDate ? new Date(order.orderDate).toLocaleString('vi-VN') : 'Đang cập nhật thời gian'}
                      </div>
                    </div>

                    <div className="text-left md:text-right">
                      <Tag color={meta.color}>{meta.label}</Tag>
                      <div className="mt-2 text-2xl font-semibold tracking-tight">{formatMoney(order.totalPrice || 0)}</div>
                    </div>
                  </div>
                    ...
                    {order.orderLineItemsList.length > 0 ? (
                      order.orderLineItemsList.map((item) => (
                        <div key={String(item.id || item.skuCode)} className="rounded-[22px] border border-[var(--color-border)] bg-white p-4">
                          ...
                          <Text className="mt-2 block font-semibold text-[var(--color-primary)]">{formatMoney(item.price * item.quantity)}</Text>
                        </div>
                      ))
                    ) : (
